...
contract GameSetupSystem is System, Constants {
    ...
    function _createMountainPath() private {
        // KPATH -> W
        uint32 open_2_west = createAction(ActionType.Open, "the path is passable", true, true, 0);
    ...
    function createAction(ActionType actionType, string memory desc, bool enabled, bool dBit, uint32 lnkId) private returns (uint32) {
        bytes32 txtId = keccak256(abi.encodePacked(desc));
        uint32 aId = _textGuid(desc);
        TxtDefStore.set(txtId, aId, TxtDefType.Action, desc);
        ActionStoreData memory actionData = ActionStoreData(actionType, txtId, enabled, dBit, lnkId);
